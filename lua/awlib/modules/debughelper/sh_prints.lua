...
do
    ...
    local function PrintType(x)
        ...
        if isentity(x) and x:IsPlayer() then return "Player" end
    ...
    local function DebugFixToString(obj, iscom)
        local ret = ""
        local rets, _ = DebugFixToStringColored(obj, iscom)
        for i = 2, #rets, 2 do
            ret = ret .. rets[i]
        end
        return ret
...
do
    ...
    function print(...)
        if not (devmode:GetBool() or ForceEnable) then
            return oldPrint(...)
        end

        local stackInfo = debug.getinfo(2)
        PrintTable(stackInfo)
        local fileName = stackInfo.short_src
        local pathData = string.Explode("/", stackInfo.short_src)
        local addonName = pathData[2] or "N/A"

        if stackInfo.short_src == "lua_run" then
            addonName = "console"
        elseif pathData[1] ~= "addons" then
            addonName = "workshop"
        end

        if addonName == "workshop" then
            fileName = stackInfo.short_src
        elseif fileAbbrev[fileName] then
            fileName = fileAbbrev[fileName]
        else
            local oldfileName = fileName
            fileName = string.Explode("/", fileName)
            fileName = fileName[#fileName]
            fileAbbrev[oldfileName] = fileName
        end

        if (not fileColors[fileName]) then
            incr = incr + 1
            fileColors[fileName] = HSVToColor(incr * 100 % 255, 1, 1)
        end

        MsgC("[" .. addonName .. "] ", fileColors[fileName], fileName .. ":" .. stackInfo.linedefined, color_white, concat(...) .. "\n")
    end
end